...
// Calyx
//
// Singleton bridge between CalyxMCPServer (network layer) and the window controller
// (app layer). MCP tool handlers call TerminalControlBridge.shared; the active
// CalyxWindowController registers itself as the delegate.
...
/// Singleton that connects CalyxMCPServer tool handlers to the active window controller.
///
/// The active `CalyxWindowController` sets itself as `delegate` on creation and
