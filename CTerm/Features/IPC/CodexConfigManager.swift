...
// CTerm
//
// Manages reading/writing ~/.codex/config.toml for the CTerm IPC MCP server.
...
struct CodexConfigManager: Sendable {
    ...
    static func enableIPC(port: Int, token: String, configPath: String? = nil) throws {
        ...
        // Remove existing cterm-ipc sections, normalize line endings
    ...
    /// Remove all `[mcp_servers.cterm-ipc]` sections from the content.
    /// Normalizes `\r\n` to `\n`.
        ...
        for line in lines {
            ...
            if inSection {
                if isAnyTableHeader(line) {
                    // Hit the next table header — end of cterm-ipc section
                    inSection = false
                    result.append(line)
                }
