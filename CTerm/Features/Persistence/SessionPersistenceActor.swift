...
private let logger = Logger(subsystem: "com.legato3.cterm", category: "SessionPersistence")
...
actor SessionPersistenceActor {
    ...
    init() {
        let ctermDir: URL
        if let testDir = ProcessInfo.processInfo.environment["CTERM_UITEST_SESSION_DIR"] {
            ctermDir = URL(fileURLWithPath: testDir, isDirectory: true)
        } else {
            let homeDir = FileManager.default.homeDirectoryForCurrentUser
            ctermDir = homeDir.appendingPathComponent(".cterm", isDirectory: true)
        }

        try? FileManager.default.createDirectory(at: ctermDir, withIntermediateDirectories: true, attributes: [
            .posixPermissions: 0o700
        ])

        self.savePath = ctermDir.appendingPathComponent("sessions.json")
        self.backupPath = ctermDir.appendingPathComponent("sessions.json.bak")
        self.recoveryMarkerPath = ctermDir.appendingPathComponent(".recovery")
    }
    ...
    /// Migrate session data from legacy Application Support path to ~/.cterm/.
