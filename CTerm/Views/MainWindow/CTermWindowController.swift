...
private let logger = Logger(
    ...
    category: "CTermWindowController"
)

@MainActor
class CTermWindowController: NSWindowController, NSWindowDelegate {
    ...
    init(window: NSWindow, windowSession: WindowSession, restoring: Bool = false, mcpServer: CTermMCPServer = .shared) {
    ...
    private func setupShortcutManager() {
        guard let ctermWindow = window as? CTermWindow else { return }
        ...
        ctermWindow.shortcutManager = manager
    }
    ...
    private func setupCommandRegistry() {
        ...
        commandRegistry.register(Command(id: "cli.install", title: "Install CLI to PATH", category: "System") {
            ...
            let cliSource = "\(appPath)/Contents/Resources/bin/cterm"
            let cliDest = "/usr/local/bin/cterm"
            ...
            if let appleScript = NSAppleScript(source: script) {
                ...
                if let error {
                    ...
                    alert.informativeText = "The 'cterm' command is now available. Run 'cterm browser --help' to get started."
    ...
    private func sendComposeText(_ text: String) -> Bool {
        // Clear predictive UI when the user commits a prompt, but keep
    ...
    @objc private func handleIPCLaunchWorkflowNotification(_ notification: Notification) {
        guard let event = CTermIPCLaunchWorkflowEvent.from(notification) else { return }
...
extension CTermWindowController: TerminalControl {
