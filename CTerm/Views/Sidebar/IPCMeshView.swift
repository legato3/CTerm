...
private struct MeshNode {
    ...
    let isHub: Bool            // true for cterm-app
...
struct IPCMeshView: View {
    ...
    private func drawNode(ctx: GraphicsContext, node: MeshNode, phase: Double) {
        ...
        // Label below node
        let label = node.isHub ? "CTerm" : shortName(node.peer.name)
        var text = Text(label)
    ...
    private func layoutNodes(in size: CGSize) -> [MeshNode] {
        ...
        let hubPeerID = CTermMCPServer.shared.appPeerID
    ...
    private var externalPeers: [Peer] {
        visiblePeers.filter { $0.name != "cterm-app" }
    }
    ...
    private func nodeTooltip(_ node: MeshNode) -> String {
        guard !node.isHub else { return "CTerm (this app)" }
...
private struct MessageMiniRow: View {
    ...
    private func shortName(_ name: String) -> String {
        String(name
            .replacingOccurrences(of: "claude-", with: "")
            .replacingOccurrences(of: "cterm-", with: "")
            .prefix(14))
    }
