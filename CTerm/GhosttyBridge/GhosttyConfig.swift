...
private let logger = Logger(subsystem: "com.legato3.cterm", category: "GhosttyConfig")
...
final class GhosttyConfigManager {
    ...
    /// Keys that CTerm manages (overrides from user's ghostty config).
    ...
    private static var ctermConfigDir: URL {
        FileManager.default.homeDirectoryForCurrentUser
            .appendingPathComponent(".config/cterm", isDirectory: true)
    }

    private static var ctermGlassPresetURL: URL {
        ctermConfigDir.appendingPathComponent("cterm-glass.conf", isDirectory: false)
    }

    private static var ctermUserSettingsURL: URL {
        ctermConfigDir.appendingPathComponent("cterm-user-settings.conf", isDirectory: false)
    }

    private static var ctermRuntimeOverrideURL: URL {
        ctermConfigDir.appendingPathComponent("cterm-runtime.conf", isDirectory: false)
    }

    /// Write user-configured terminal overrides to cterm-user-settings.conf.
    ...
    static func writeUserSettings(_ overrides: [String: String]) {
        ...
        do {
            try fm.createDirectory(at: ctermConfigDir, withIntermediateDirectories: true)
            guard !overrides.isEmpty else {
                try? fm.removeItem(at: ctermUserSettingsURL)
                return
            }
            var lines = ["# --- CTerm User Settings (managed by CTerm Settings UI) ---"]
            for (key, value) in overrides.sorted(by: { $0.key < $1.key }) {
                lines.append("\(key) = \(value)")
            }
            lines.append("# --- End CTerm User Settings ---")
            try (lines.joined(separator: "\n") + "\n").write(to: ctermUserSettingsURL, atomically: true, encoding: .utf8)
        } catch {
    ...
    /// Note: configLoadFile for cterm-glass.conf runs on EVERY loadDefaultConfig call regardless of this flag.
    ...
    static func loadDefaultConfig() -> ghostty_config_t? {
        ...
        if !hasAppliedPreset {
            applyCTermGlassPresetIfPossible()
        ...
        // Load CTerm glass preset after default files so its values take precedence.
        let presetPath = ctermGlassPresetURL.path
        ...
        let userSettingsPath = ctermUserSettingsURL.path
    ...
    private static func applyRuntimeOverrides(_ cfg: ghostty_config_t) {
        ...
        do {
            ...
            let text = """
            # --- CTerm Runtime Override (managed) ---
            ...
            try (finalText + "\n").write(to: ctermRuntimeOverrideURL, atomically: true, encoding: .utf8)
            GhosttyFFI.configLoadFile(cfg, path: ctermRuntimeOverrideURL.path)
        } catch {
            logger.warning("Failed to apply runtime overrides: \(error.localizedDescription, privacy: .public)")
        }
    }

    private static func applyCTermGlassPresetIfPossible() {
        let fm = FileManager.default
        let presetConfigURL = ctermGlassPresetURL

        do {
            // 1. Create ~/.config/cterm/ directory if needed.
            try fm.createDirectory(at: ctermConfigDir, withIntermediateDirectories: true)
            ...
            if let oldBundleID = Bundle.main.bundleIdentifier,
               let oldAppSupport = fm.urls(for: .applicationSupportDirectory, in: .userDomainMask).first {
                let oldPath = oldAppSupport
                    ...
                    .appendingPathComponent("cterm-glass.conf", isDirectory: false)
            ...
            // 3. Create cterm-glass.conf with defaults if it doesn't exist.
            if !fm.fileExists(atPath: presetConfigURL.path) {
                ...
                // 4. Remove deprecated keys from existing cterm-glass.conf (migration).
            ...
            logger.warning("Failed to apply CTerm glass preset: \(error.localizedDescription, privacy: .public)")
        }
    }

    /// Removes any stale CTerm-managed blocks from the main ghostty config file.
    ...
    private static func cleanupMainGhosttyConfig(fileManager fm: FileManager) {
        ...
        // Remove all known CTerm-managed block types.
        cleaned = removeManagedBlock(
            from: cleaned,
            startMarker: "# --- CTerm Visual Defaults (managed) ---",
            endMarker: "# --- End CTerm Visual Defaults ---"
        )
        cleaned = removeManagedBlock(
            from: cleaned,
            startMarker: "# --- CTerm Glass Preset (managed) ---",
            endMarker: "# --- End CTerm Glass Preset ---"
        )
        cleaned = removeManagedBlock(
            from: cleaned,
            startMarker: "# --- CTerm Include (managed) ---",
            endMarker: "# --- End CTerm Include ---"
        )
        ...
        if cleaned != existingMain {
            do {
                ...
                logger.info("Cleaned stale CTerm managed blocks from ghostty config")
            } catch {
