...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    private var windowControllers: [CTermWindowController] = []
    ...
    var allWindowControllers: [CTermWindowController] {
        windowControllers
    }
    ...
    @objc func createNewWindow() {
        ...
        let wc = CTermWindowController(windowSession: windowSession)
    ...
    func removeWindowController(_ controller: CTermWindowController) {
    ...
    func isClosingLastManagedWindow(_ controller: CTermWindowController) -> Bool {
    ...
    func closingWouldTerminate(_ controller: CTermWindowController) -> Bool {
    ...
    private func setupMainMenu() {
        ...
        appMenu.addItem(withTitle: "About CTerm", action: #selector(showAboutPanel), keyEquivalent: "")
        ...
        appMenu.addItem(.separator())
        appMenu.addItem(withTitle: "Hide CTerm", action: #selector(NSApplication.hide(_:)), keyEquivalent: "h")
        ...
        fileMenu.addItem(withTitle: "New Tab", action: #selector(CTermWindowController.newTab(_:)), keyEquivalent: "t")
        fileMenu.addItem(withTitle: "New Browser Tab", action: #selector(CTermWindowController.newBrowserTab(_:)), keyEquivalent: "")
        fileMenu.addItem(.separator())
        fileMenu.addItem(withTitle: "Close Tab", action: #selector(CTermWindowController.closeTab(_:)), keyEquivalent: "w")
        ...
        let composeItem = NSMenuItem(
            title: "Compose Input",
            action: #selector(CTermWindowController.toggleComposeOverlay),
            keyEquivalent: "e"
        )
        ...
        let toggleSidebarItem = NSMenuItem(
            ...
            action: #selector(CTermWindowController.toggleSidebar),
            keyEquivalent: "s"
        )
        ...
        let nextTabItem = NSMenuItem(title: "Select Next Tab", action: #selector(CTermWindowController.selectNextTab(_:)), keyEquivalent: "]")
        nextTabItem.keyEquivalentModifierMask = [.command, .shift]
        windowMenu.addItem(nextTabItem)

        let prevTabItem = NSMenuItem(title: "Select Previous Tab", action: #selector(CTermWindowController.selectPreviousTab(_:)), keyEquivalent: "[")
        prevTabItem.keyEquivalentModifierMask = [.command, .shift]
        windowMenu.addItem(prevTabItem)

        let jumpUnreadItem = NSMenuItem(title: "Jump to Unread Tab", action: #selector(CTermWindowController.jumpToMostRecentUnreadTab), keyEquivalent: "u")
    ...
    private func restoreWindow(_ windowSnap: WindowSnapshot) -> Bool {
        ...
        let wc = CTermWindowController(window: window, windowSession: windowSession, restoring: true)
        windowControllers.append(wc)
    ...
    private func startIPCIfNeeded() {
        guard UserDefaults.standard.bool(forKey: "cterm.ipcAutoStart") else { return }
        guard !CTermMCPServer.shared.isRunning else { return }

        let token = SecurityUtils.generateHexToken()

        do {
            try CTermMCPServer.shared.start(token: token)
            let port = CTermMCPServer.shared.port
