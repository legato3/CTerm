//
//  CTermMCPServerTests.swift
//  CTermTests
//
//  Tests for CTermMCPServer: the MCP server's handleJSONRPC method
...
@MainActor
final class CTermMCPServerTests: XCTestCase {

    // MARK: - Properties

    private var server: CTermMCPServer!
    ...
    override func setUp() {
        super.setUp()
        server = CTermMCPServer(testToken: testToken)
    }
    ...
    func test_handleJSONRPC_initialize_returnsInitResult() async throws {
        ...
        XCTAssertEqual(serverInfo["name"] as? String, "cterm-ipc",
                       "serverInfo.name must be cterm-ipc")
    ...
    // 5. "tools/list" → result with 8 tools
    func test_handleJSONRPC_toolsList_returnsAllTools() async throws {
        ...
        XCTAssertEqual(tools.count, 8,
                       "tools/list must return exactly 8 tools")
        ...
        XCTAssertEqual(toolNames, expectedNames,
                       "tools/list must return the 8 expected tool names")
    }
