...
final class ClaudeConfigManagerTests: XCTestCase {
    ...
    func test_enableIPC_createsConfigFromScratch() throws {
        ...
        // Then: file is created with correct cterm-ipc entry
        ...
        XCTAssertNotNil(ctermIPC, "cterm-ipc entry should exist")
        XCTAssertEqual(ctermIPC?["type"] as? String, "http")
        XCTAssertEqual(ctermIPC?["url"] as? String, "http://localhost:41830/mcp")
    ...
    func test_enableIPC_addsToExistingConfig() throws {
        ...
        // Then: cterm-ipc is added, other-server is preserved
        let dict = try readConfigDict()
        let mcpServers = dict["mcpServers"] as? [String: Any]
        XCTAssertNotNil(mcpServers?["cterm-ipc"], "cterm-ipc should be added")
        XCTAssertNotNil(mcpServers?["other-server"], "other-server should be preserved")
    }

    func test_enableIPC_updatesExistingEntry() throws {
        // Given: cterm-ipc already exists with old token
        let existingJSON = """
        {
            "mcpServers": {
                "cterm-ipc": {
        ...
        // Then: cterm-ipc is updated with new values
        let dict = try readConfigDict()
        let mcpServers = dict["mcpServers"] as? [String: Any]
        let ctermIPC = mcpServers?["cterm-ipc"] as? [String: Any]
        XCTAssertEqual(ctermIPC?["url"] as? String, "http://localhost:55555/mcp")
    ...
    func test_enableIPC_createsBackup() throws {
        ...
        XCTAssertFalse(bakContent.contains("cterm-ipc"),
                       "Backup should not contain the newly added cterm-ipc entry")
    }
    ...
    func test_enableIPC_correctURLFormat() throws {
        ...
        let mcpServers = dict["mcpServers"] as? [String: Any]
        let ctermIPC = mcpServers?["cterm-ipc"] as? [String: Any]
        XCTAssertEqual(ctermIPC?["url"] as? String, "http://localhost:12345/mcp",
    ...
    func test_disableIPC_removesCTermEntry() throws {
        // Given: config has cterm-ipc and another server
        let existingJSON = """
        {
            "mcpServers": {
                "cterm-ipc": {
        ...
        // Then: cterm-ipc is removed, other-server is preserved
        let dict = try readConfigDict()
        let mcpServers = dict["mcpServers"] as? [String: Any]
        XCTAssertNil(mcpServers?["cterm-ipc"], "cterm-ipc should be removed")
        XCTAssertNotNil(mcpServers?["other-server"], "other-server should be preserved")
    }

    func test_disableIPC_removesMcpServersIfEmpty() throws {
        // Given: config has only cterm-ipc in mcpServers
        let existingJSON = """
        {
            "mcpServers": {
                "cterm-ipc": {
    ...
    func test_isIPCEnabled_falseWhenAbsent() {
        // Given: config has mcpServers but no cterm-ipc
        let existingJSON = """
        {
            "mcpServers": {
                "other-server": {
