...
final class CodexConfigManagerTests: XCTestCase {
    ...
    func test_enableIPC_addsToExistingFile() throws {
        ...
        // Then: original content preserved and cterm-ipc appended
        let content = readConfig()
    ...
    func test_enableIPC_updatesExistingEntry() throws {
        // Given: file already has cterm-ipc section with old port/token
        let existing = """
    ...
    func test_disableIPC_removeSection() throws {
        // Given: file has cterm-ipc section
        let existing = """
        [mcp_servers.cterm-ipc]
        url = "http://localhost:41830/mcp"
    ...
    func test_disableIPC_preservesOtherMCPServers() throws {
        ...
        // Then: cterm-ipc removed, other preserved
        let content = readConfig()
        XCTAssertFalse(content.contains("[mcp_servers.cterm-ipc]"))
        XCTAssertTrue(content.contains("[mcp_servers.other]"))
        XCTAssertTrue(content.contains("http://localhost:9999/mcp"))
    ...
    func test_disableIPC_noSection() throws {
        // Given: file exists but has no cterm-ipc section
        let existing = """
        [general]
        model = "gpt-4"
        """
    ...
    func test_disableIPC_unreadableFile() throws {
        ...
        writeConfig("[mcp_servers.cterm-ipc]\nurl = \"http://localhost:41830/mcp\"\n")
    ...
    func test_enableIPC_duplicateSections() throws {
        // Given: file has TWO cterm-ipc sections
        let existing = """
        [mcp_servers.cterm-ipc]
        url = "http://localhost:40000/mcp"
        ...
        // Exactly one cterm-ipc section header
        let headerCount = content.components(separatedBy: "[mcp_servers.cterm-ipc]").count - 1
        XCTAssertEqual(headerCount, 1, "Should have exactly one cterm-ipc section")
    }

    func test_enableIPC_arrayOfTablesIgnored() throws {
        // Given: file has [[array_of_tables]] inside the cterm-ipc section area
        ...
        XCTAssertFalse(content.contains("[[array_of_tables]]"),
                       "[[array_of_tables]] inside cterm-ipc section should be removed")
    ...
    func test_enableIPC_inlineCommentOnSectionHeader() throws {
        // Given: cterm-ipc section header has a trailing comment
        let existing = """
        [mcp_servers.cterm-ipc] # added by CTerm
        url = "http://localhost:40000/mcp"
        http_headers = { "Authorization" = "Bearer old" }
        """
        writeConfig(existing)
    ...
    func test_concurrentEnableDisable_noCorruption() throws {
        ...
        if FileManager.default.fileExists(atPath: configPath) {
            ...
            // If cterm-ipc section exists, it should have the url key
            if content.contains("[mcp_servers.cterm-ipc]") {
                XCTAssertTrue(content.contains("url = \"http://localhost:"),
                              "cterm-ipc section should have url key")
            }
        }
    ...
    func test_removeSections_preservesMultiLineStringContent() throws {
        // Given: config with a multi-line string whose content looks like a cterm-ipc header
        let content = """
        [other]
        matrix = \"\"\"
        [mcp_servers.cterm-ipc]
        ...
        // Then: the real cterm-ipc section is removed, but string content is intact
        let result = readConfig()
        XCTAssertFalse(result.contains("url = \"http://localhost:41830/mcp\""),
                       "Real cterm-ipc section should be removed")
