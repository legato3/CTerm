...
final class GroupManagementUITests: CTermUITestCase {
    ...
    func test_createNewGroup() {
        ...
        let palette = app.descendants(matching: .any)
            .matching(identifier: "cterm.commandPalette")
            .firstMatch
        waitForNonExistence(palette)

        // Should now have 2 groups
        Thread.sleep(forTimeInterval: 0.5)
        let groupCount = countElements(matching: "cterm.sidebar.group.", excludingSuffix: "Button")
    ...
    func test_switchGroup() {
        ...
        let groups = app.descendants(matching: .any)
            .matching(NSPredicate(format: "identifier BEGINSWITH %@ AND NOT identifier ENDSWITH %@", "cterm.sidebar.group.", "Button"))
    ...
    func test_renameGroupByDoubleClick() {
        ...
        let groups = app.descendants(matching: .any)
            .matching(NSPredicate(format: "identifier BEGINSWITH %@", "cterm.sidebar.group."))
        ...
        let renameField = app.descendants(matching: .any)
            .matching(NSPredicate(format: "identifier BEGINSWITH %@", "cterm.sidebar.groupNameTextField."))
            .firstMatch
        XCTAssertTrue(
    ...
    func test_collapseGroupHidesTabs() {
        ...
        let initialTabCount = countElements(matching: "cterm.sidebar.tab.")
        ...
        let collapseButton = app.descendants(matching: .any)
            .matching(NSPredicate(format: "identifier BEGINSWITH %@", "cterm.sidebar.groupCollapseButton."))
            .firstMatch
        XCTAssertTrue(
        ...
        let tabCountAfterCollapse = countElements(matching: "cterm.sidebar.tab.")
        XCTAssertEqual(
    ...
    func test_expandGroupShowsTabs() {
        ...
        let tabCountCollapsed = countElements(matching: "cterm.sidebar.tab.")
        XCTAssertEqual(
            tabCountCollapsed, 0,
            "All tabs should be hidden after collapsing the group"
        )
        ...
        let tabCountExpanded = countElements(matching: "cterm.sidebar.tab.")
        XCTAssertGreaterThanOrEqual(
            tabCountExpanded, 1,
    ...
    func test_consecutiveGroupCloseButtons() {
        ...
        let initialGroupCount = countElements(matching: "cterm.sidebar.group.", excludingSuffix: "Button")
        ...
        let firstGroup = app.descendants(matching: .any)
            .matching(NSPredicate(format: "identifier BEGINSWITH 'cterm.sidebar.group.' AND NOT identifier CONTAINS '.closeAllButton' AND NOT identifier CONTAINS '.collapseButton'"))
            .element(boundBy: 0)
        firstGroup.hover()
        Thread.sleep(forTimeInterval: 0.3)
        ...
        XCTAssertEqual(
            countElements(matching: "cterm.sidebar.group.", excludingSuffix: "Button"),
            2,
